...
#undef min
#undef max
#define RETURN_NULL_REF(x) return (**(x**)NULL)
...
struct building: public object
{
	...
	virtual ~building() {}
	virtual creep_tumor& a_creep_tumor_i_suppose() { assert(0 && "not a creep tumor"); RETURN_NULL_REF(creep_tumor); }
};
...
struct unit: public object
{
	...
	virtual ~unit() {}
	virtual queen& a_queen_i_suppose() { assert(0 && "not a queen"); RETURN_NULL_REF(queen); }
};
	...
	friend std::ostream &operator<< (std::ostream &o, game const &g)
	{
		...
		for(uint y=g.map_dy; y--;)
		{
			...
			for(uint x=0; x<g.map_dx; ++x)
			{
				...
				else { o << print_empty; empty_space++; }
			}
...
int main(int argc, char **argv)
{
	...
	while(g.anything_to_do() && g.t_q2<g.t_limit_q2)
		step(g,false);
	std::cout << g;
	std::cout << "EMPTY: " << empty_space << std::endl;
	std::cout << "TIME: " << g.t_q2 << std::endl;
	return 0;
}
